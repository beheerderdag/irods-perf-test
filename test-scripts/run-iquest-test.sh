...
{ date &  echo "== Q6 data path query with count ==" ;  }  >>$outputfilename ;  /usr/bin/time -f "%e" iquest --no-page "SELECT count(DATA_NAME)  WHERE DATA_PATH like  '%test%'" 2>> $outputfilename
...
{ date &  echo "== Q9 meta data filter by value ==" ;   } >> $outputfilename ; /usr/bin/time -f "%e"   iquest --no-page "SELECT META_DATA_ATTR_NAME , META_DATA_ATTR_VALUE where META_DATA_ATTR_VALUE like '%blue.%'" 2>> $outputfilename 
...
{ date &  echo "== Q11 resource name and meta data look up ==" ; } >> $outputfilename ; /usr/bin/time -f "%e"  iquest --no-page "select COLL_NAME,DATA_NAME where COLL_NAME like '%perf%' AND RESC_NAME = 'MyResc' and AND META_DATA_ATTR_VALUE like '2%'" 2>>$outputfilename 

{ date &  echo "== Q12 OR filter meta data value =="    ;        } >> $outputfilename ; /usr/bin/time -f "%e"  iquest --no-page "SELECT META_DATA_ATTR_NAME , META_DATA_ATTR_VALUE where META_DATA_ATTR_VALUE like '%blue.%' || like 'green'" 2>>$outputfilename
